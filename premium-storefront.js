(function () {
  ...
  function shareProduct(product) {
    ...
    var data = {
      title: product.name + " · Kurti Aura",
      text: product.name + " — ₹" + (product.price || 500) + " at Kurti Aura",
      url: url
    };
    if (navigator.share) {
  ...
  function ensureProductActions(product, details) {
    ...
    var url = productShareUrl(product);
    var message = "See this " + product.name + " at Kurti Aura — ₹" + (product.price || 500) + ": " + url;
    actions.innerHTML = [
  ...
  function showInstallBanner() {
    ...
    banner.setAttribute("aria-label", "Install Kurti Aura app");
    banner.innerHTML = [
      '<span class="aura-install-mark">K</span>',
      "<div><strong>Keep Kurti Aura on your phone</strong><small>Faster access from your home screen</small></div>",
  ...
  function checkoutItems(form) {
    ...
    return Array.from(checkout.querySelectorAll(".order-review .review-item")).map(function (item) {
      ...
      return ((name ? name.textContent.trim() : "Kurti Aura item") + (detail ? " — " + detail.textContent.trim() : "")).trim();
    }).filter(Boolean);
  }
  ...
  function whatsappOrderMessage(data) {
    return [
      "*KURTI AURA — NEW ORDER*",
      "",
      "*CUSTOMER DETAILS*",
  ...
  function openWhatsAppOrder(form) {
    ...
    try {
      ...
      var message = whatsappOrderMessage(order);
      var whatsappUrl = "https://wa.me/" + STORE_WHATSAPP_NUMBER + "?text=" + encodeURIComponent(message);
      window.location.assign(whatsappUrl);
    } catch (error) {
      setOrderError(form, error && error.message ? error.message : "Order could not be sent. Please try again.");
      showToast("Please check your order details and try again");
    } finally {
      if (button) {
  ...
  function registerApp() {
    ...
    window.addEventListener("appinstalled", function () {
      ...
      showToast("Kurti Aura added to your home screen");
    });
  }
